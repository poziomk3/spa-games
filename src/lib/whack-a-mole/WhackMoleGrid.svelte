...
<div class="col-span-10 flex flex-col justify-center">
  <div class="text-center pb-3">{numberToRender - produced} moles left!</div>
  <div
    style="--fieldNumber:{fieldNumber}"
    class="flex-1 relative grid column-number aspect-square min-h-0 mx-auto gap-[0.5rem]"
  >
    {#if !isRunning}
      <div class="absolute w-full h-full bg-black/80"></div>{/if}
